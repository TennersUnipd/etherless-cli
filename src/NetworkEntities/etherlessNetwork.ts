...
export default class EtherlessNetwork extends NetworkInterface {
   ...
   * constructor
   */
  public constructor(provider: any, remoteServer: string) {
    super(remoteServer);
    this.web3 = provider;
  }
  ...
  public uploadFunction(fileBuffer: string, ename: string, service: string): Promise<any> {
    return axios.post(this.endpoint + service,
      {
        zip: fileBuffer,
        name: ename,
      });
