...
class SetCommand extends Command {
    ...
    exec(inputs: SetCommandInputs): Promise<any> {
      return this.network.setFunctionProperty(inputs.function, inputs.property, inputs.newValue, inputs.password).then(response => {
        return "Done";
      });
    }
    ...
    parseArgs(args: string[]): CommandInputs {
      return { function: args[0], property: args[1], newValue: args[2], password: args[3] };
    }
}
